...
export default function ChangePasswordScreen() {
  ...
  const handleUpdatePassword = async () => {
    ...
    if (password.length < 6) {
        setToast({ visible: true, message: "⚠️ La contraseña debe tener mín. 6 caracteres", type: 'error' });
        return;
    }
    
    setLoading(true);

    // Actualizamos el usuario (ya autenticado por el paso anterior)
    ...
    if (error) {
      ...
      setToast({ visible: true, message: "🔒 ¡Contraseña actualizada!", type: 'success' });
    ...
    <View style={styles.container}>
      <View style={styles.content}>
        ...
        <View style={styles.iconCircle}>
          {/* Usamos el color de marca para consistencia */}
...
const styles = StyleSheet.create({
  ...
  inputFocused: { borderColor: '#818CF8', backgroundColor: '#1A1A2E' }, // Efecto focus
